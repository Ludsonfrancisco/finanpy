...
abstract interface class CardsRepository {
  ...
  Future<CardDetailSnapshot> fetchCardDetail(int cardId, {int? month, int? year});
...
final class HttpCardsRepository implements CardsRepository {
  ...
  Future<CardsSnapshot> fetchCards({int? month, int? year, String? owner}) async {
    ...
    final uri = Uri(path: '/api/v1/cards/', queryParameters: queryParams.isEmpty ? null : queryParams);
    ...
    return CardsSnapshot(
      cards: cardsJson.map((c) => CreditCardModel.fromJson(c as Map<String, dynamic>)).toList(),
  ...
  Future<CardDetailSnapshot> fetchCardDetail(int cardId, {int? month, int? year}) async {
    ...
    final uri = Uri(path: '/api/v1/cards/$cardId/', queryParameters: queryParams.isEmpty ? null : queryParams);
    final response = await _transport.getObject(uri.toString());

    final cardJson = (response['card'] as Map<String, dynamic>?) ?? {};
    final invoiceJson = (response['selected_invoice'] as Map<String, dynamic>?) ?? {};
    final futureInvoicesJson = (response['future_invoices'] as List<dynamic>?) ?? [];

    return CardDetailSnapshot(
      card: CreditCardModel.fromJson(cardJson),
      selectedInvoice: CardInvoiceModel.fromJson(invoiceJson),
      futureInvoices: futureInvoicesJson.map((i) => CardInvoiceModel.fromJson(i as Map<String, dynamic>)).toList(),
    );
  }
  ...
  Future<CreditCardModel> createCard({
    ...
    final response = await _transport.postObject('/api/v1/cards/', data: {
      'name': name,
      'limit': limit,
  ...
  Future<CreditCardModel> updateCard(
    ...
    final response = await _transport.patchObject('/api/v1/cards/$id/', payload);
    return CreditCardModel.fromJson(response);
  }
  ...
  Future<List<CardExpenseModel>> createExpense({
    ...
    final response = await _transport.postObject('/api/v1/cards/expenses/', data: payload);
    final expensesJson = (response['expenses'] as List<dynamic>?) ?? [];
    return expensesJson.map((e) => CardExpenseModel.fromJson(e as Map<String, dynamic>)).toList();
  }

  @override
  Future<void> deleteExpense(int expenseId, {bool deleteAll = false}) async {
    final path = deleteAll ? '/api/v1/cards/expenses/$expenseId/?delete_all=true' : '/api/v1/cards/expenses/$expenseId/';
  ...
  Future<CardInvoiceModel> payInvoice(
    ...
    final dateStr = '${paymentDate.year.toString().padLeft(4, '0')}-${paymentDate.month.toString().padLeft(2, '0')}-${paymentDate.day.toString().padLeft(2, '0')}';
    final response = await _transport.postObject('/api/v1/cards/invoices/$invoiceId/pay/', data: {
  ...
  Future<CardInvoiceModel> reopenInvoice(int invoiceId) async {
    final response = await _transport.postObject('/api/v1/cards/invoices/$invoiceId/reopen/', data: {});
    return CardInvoiceModel.fromJson(response);
  }
}
